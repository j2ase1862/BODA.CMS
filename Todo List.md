...
## 0. UR (4호 벤더 — 드라이버·카탈로그 완료, §5.5)

- [ ] **URSim 파이프라인 통과 확인**(§6 7단계) — 도커 `universalrobots/ursim_e-series` 기동 →
      수집 → TimescaleDB 적재 → 대시보드 표시 (도커 가능한 PC에서, 실기 불필요)
- [ ] 🔶 실기 확보 → 실기 매핑·검증(§6 3·4단계): 실 주기 확인(e-Series 500Hz 상향 검토),
      `actual_current` 부호·단위 sanity, `target_moment` 값 대조 → capability 검증(Pro 판정 확정)
- [ ] 실측·펌웨어 호환성 노트 기록 (ROADMAP §5.5, §6 8단계)
...
- [ ] 후보 벤더 조사 (Techman 등 — UR은 §5.5로 착수 완료) — 시장 요구 발생 시 §6 절차로
