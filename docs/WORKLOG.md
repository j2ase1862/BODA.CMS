...
## 2026-09-21 (40) — 두산 J1 방향 실기 확인 + "베이스 방위 180°" 항목 종결

### J1 = +1 확정 (실기)
사용자가 J1 을 +90° 조그하니 3D 도 같은 방향으로 돌았다. `RobotViewPostureTests` 에 방위각 테스트를 추가해 고정했다 —
J1 자세에서 +90° 조그 시 플랜지 방위각(`atan2(x, z)`)이 +90° 증가. 부호표만 보는 테스트와 달리 **J1 축 벡터까지** 묶는다
(축을 `(0,-1,0)` 으로 바꿔 보니 이 테스트만 깨지는 것을 확인). 래핑 주의: 차이를 (-180, 180] 로 정규화해야 한다.

### "베이스 방위 180° 의심"은 문제가 아니었다 — 항목을 닫는다

#38 에서 펜던트 태스크 `X=-731.09` 와 모델의 +X 전개가 반대라 열어 둔 항목인데, 두 가지 이유로 추적할 실익이 없다.

1. **모델 X 는 베이스 X 가 아니다.** `BuildRobot` 이 카메라 정렬용으로 루트에 요 45° 를 이미 넣는다(형상 무관). 방위 기준이
   애초에 임의값이라 "180° 어긋났다"는 말 자체가 성립하지 않는다.
2. **구분되는 차이가 공구 자세가 아니다.** 지금 부호 `{1,1,1,1,1,1}`(A)와 대안 `{1,-1,-1,1,-1,1}`(B)를 여러 자세에서 비교하면
   **공구축 방향이 모든 자세에서 완전히 동일**(차이 0.0000)하고, A 를 수직축으로 180° 돌린 것과 B 의 차이는 **링크 측면
   오프셋이 반대쪽에 붙는 것뿐**이다(팔꿈치 0.17m, 플랜지 0.04m). 그 측면 오프셋은 UR5e ×0.62 에서 가져온 개략값이라
   두산 실물과 대조할 기준이 못 된다(이 뷰의 치수는 원래 실제 기구학이 아니라 일반 비례다).

즉 CBM 판정에도, 공구 자세 표시에도 영향이 없다. 나중에 **베이스 좌표축 표시나 워크셀 배치**를 넣게 되면 그때 다시 정하면 된다.

### 남은 확인 (실기)
- **J4·J6(롤 축) 방향** — 유일하게 남은 방향 미검증 항목이고, 여기가 틀리면 롤이 들어간 자세에서 공구 방향이 실제로 어긋난다.
  손목 롤 90° 자세 한 번이면 된다.
- v0.7.7 → v0.7.8 인앱 [지금 설치] 자동 경로 (#35 부터 미확인).
- Collector 설치본 `sc qc BODA.CMS.Collector` → `AUTO_START (DELAYED)` (#37 잔여).
- 현장 `J2 전류A z=5748.9` — 정지 중 학습된 기준선. J2 기준선 재학습 필요(별건, 사용자가 뒤로 미룸).

테스트 118/118 통과.

---

## 2026-09-21 (39) — v0.7.8 패키징·릴리스 발행 (두산 J5 회귀 수정)

- 담긴 변경: #38 — 두산 J5 반전 철회 + 자세 기하 테스트(`RobotViewPostureTests`). v0.7.7 회귀 수정이라 현장 업데이트가 필요하다.
- 패키징 `tools\package.ps1 -Version 0.7.8` (기준선도 0.7.8 이라 경고 없음), WiX 경고 0. 산출물 5종.
  각인 확인: zip 안 `BODA.CMS.dll` 의 `ProductVersion = 0.7.8+f43eab017220…` = HEAD(`f43eab0`, 작업 트리 깨끗) 일치,
  앱 MSI `ProductVersion = 0.7.8`.
- 발행: `gh release create v0.7.8 --repo j2ase1862/CMS-Releases` — 5개 파일 **명시** 지정.
  검증: API `releases/latest` = **v0.7.8**, 자산 5종 `uploaded`,
  앱 MSI digest `sha256:cf090e76039a40f52ef0317f4e245d1712b4e13b0f8a2397e12463cce17a30cc` = 로컬 SHA-256 일치.
- 보관: `D:\CMS-Releases` 에 5종 복사, `dist` 의 0.7.5~0.7.7 산출물 삭제.
- 기준선은 **0.7.9** 로 올렸다(발행 직후 상향).

### 릴리스 노트에 적은 것
v0.7.7 의 3D 손목 방향 회귀 · **각도 값·CBM 판정에는 영향 없음**(자세 표시만) · 베이스 방위와 J4·J6 롤은 대조 중 ·
이번 변경은 모니터 앱만 해당(감시 서버는 v0.7.7 과 기능 동일).

### 실기 확인 완료
- **v0.7.8 설치 후 3D 자세가 실기와 일치**(사용자 확인, 2026-09-21). #38 의 J5 철회가 맞았다는 현장 검증이다.
  v0.7.7 의 반전은 완전히 접는다 — 두산은 전 축 보고값 그대로.

### 남은 확인 (실기)
- #38 의 두 가지: 베이스 방위 180° 의심(J1 +90° 조그로 판정) · J4·J6 롤 방향(롤 90° 자세에서).
  자세 모양은 맞는 것으로 확인됐으니 둘 다 "모양"이 아니라 **도는 방향**을 봐야 잡힌다.
- v0.7.7 → v0.7.8 인앱 [지금 설치] 자동 경로 — #35 부터 계속 미확인. 양쪽 다 2단계 코드가 있어 이번에도 확인 가능하다.
- Collector 설치본 `sc qc BODA.CMS.Collector` → `START_TYPE : 2 AUTO_START (DELAYED)` (#37 잔여).
- 현장 `J2 전류A z=5748.9` — 정지 중 학습된 기준선으로 보인다. J2 기준선 재학습 필요(3D 와 무관한 별건).

---

## 2026-09-21 (38) — 두산 J5 반전 철회 (v0.7.7 이 방향을 거꾸로 잡았다) + 자세 기하 테스트

### 무엇이 틀렸나

현장 사진 4장(펜던트 · 실기 · CMS 3D 2장, v0.7.7 구동)을 대조했다.

- **각도 값은 정상** — 펜던트 조인트 `0.01 / 0.07 / -89.93 / 0.01 / -90.03 / -0.01` 과 CMS `위치°` 가 소수점까지 동일. 드라이버 문제가 아니다.
- **자세는 손목 한 군데가 반대** — 실기는 상완 수직 → 팔꿈치 90° → 전완 수평 → **손목이 아래로 꺾여 흡착 그리퍼가 바닥**을 보는데,
  v0.7.7 의 3D 는 손목을 **위로** 그렸다(그리퍼 손가락이 하늘 방향). 팔(J1~J3)은 일치.

### 원인 — #36 의 반전이 방향을 거꾸로 잡았다

모델의 변환 체인을 그대로 풀어 공구축 방향을 계산하면(q = 현장 값, 롤-피치-롤 손목):

| J5 부호 | 플랜지 y | 공구축 | |
|---|---|---|---|
| +1 (반전 없음) | 0.371 | (0, **-1**, 0) | 아래 — 실기와 일치 |
| -1 (v0.7.7) | 0.481 | (0, **+1**, 0) | 위 — 사진의 CMS 화면과 일치 |

즉 **반전을 넣기 전이 맞았다.** `DoosanFlips` 를 지우고 두산도 `NoFlips` 를 쓴다 — 두산은 전 축 보고값 그대로다.

### 왜 #36 에서 걸러지지 않았나 (테스트의 교훈)

`RobotViewProfileTests` 는 `ModelAngle` **부호 산술만** 봤다. 부호가 반대여도 "J5 만 -1" 단정은 통과한다 —
자세가 틀렸는지는 **공구가 어디를 향하는지**로만 판정할 수 있다. 그래서:

- 관절 위치·축을 `ChainFor(WristKind)` 표로 빼고(`JointFrame` = 회전축 + 부착 지점), 변환 합성은 `JointTransform` 하나로 모았다.
  모델(`BuildRobot`)과 테스트가 **같은 표·같은 합성 순서**를 쓴다. 형상 숫자(반지름 등)는 그대로 빌더에 남겼다.
- `ToolTipOffset(WristKind)` = 플랜지 로컬 기준 그리퍼 끝 — 공구축 판정 지점.
- `RobotViewPostureTests` 3건 추가: 두산 영점 = 수직 상향, **두산 J5=-90 → 공구축 아래**(실기 값 그대로),
  UR 영점 오프셋 = 전완 수평. J5 를 -1 로 되돌리면 두 번째가 실제로 깨지는 것을 확인했다.
- `RobotViewProfileTests` 의 두산 항목은 "전 축 보고값 그대로"로 교체.

테스트 117/117 통과, Release 빌드 경고 0.

### 남은 확인 (실기)

- **베이스 방위 180° 의심** — 펜던트 태스크 좌표가 `X=-731.09` 인데 모델은 같은 각도에서 전완이 모델 **+X** 로 뻗는다.
  두산 베이스 기준이면 J3=-90 은 -X 여야 한다. 화면에 베이스 좌표축 표시가 없어 자세 모양만으로는 판별이 안 되고
  (J2·J3·J5 를 한꺼번에 뒤집은 해와 지금 해가 화면상 같은 모양이다), **J1 을 +90° 조그해서 모델이 같은 쪽으로 도는지**
  보면 확정된다. 추측으로 넣지 않았다.
- J4·J6(롤 축) 방향 — 이번 자세는 둘 다 0° 라 대조가 안 됐다. 손목 롤을 90° 준 자세에서 한 번 봐야 한다.
- 실기에서 `J2 전류A z=5748.9` 알림 — z 가 5천대면 기준선 표준편차가 사실상 0, 정지 중에 학습된 기준선으로 보인다. 3D 와 무관한 별건.
...
- `RobotStatusView`: 3D : 히트맵/추세 열을 1* : 2*(3D 최소 300)로 — 창 폭에 비례해 함께 커진다.
  - 추세선(Polyline): 고정 190px → Stretch, 점 좌표는 레이아웃이 준 `ActualWidth` 기준(창 크기 바뀌면 다음 틱에 재계산).
  - 히트맵 셀 열: 고정 32px → 별 크기(32~96px), 바깥 Grid 열 `HeatColumn.MaxWidth = 64 + 96×축수` 로 상한·왼쪽 정렬.
  - 3D 뷰포트: 높이를 폭에 비례(300:235, 상한 420) — WPF `PerspectiveCamera.FieldOfView` 는 수평 화각이라 폭만 늘면 로봇 위가 잘리던 것 해결.
- 표 모드 판독 표는 Consolas 텍스트 블록이라 폭 개념이 없음(패널은 이미 Stretch) — 변경 없음.
- 검증: 최대화(2560×1600) 시뮬레이터 로봇 모드 캡처 — 추세선이 카드 폭 끝까지, 히트맵 셀 96px, 3D 잘림 없음. 빌드 경고 0, 테스트 109/109.
...
- `Directory.Build.props`(리포 루트) `<Version>0.7.5</Version>` — 앱·Collector·드라이버·테스트 공통. Debug 빌드 ProductVersion 0.7.5+<sha> 확인(체커의 현재 버전 = 0.7.5).
- `tools/package.ps1`: `-Version` 미지정 시 기준선 사용, 지정 값이 기준선과 다르면 경고(발행 후 기준선 갱신 누락 방지). 릴리스 절차에 "발행 후 기준선을 다음 버전으로 올린다" 추가.
- 빌드 경고 0, 테스트 109/109.

---

## 2026-09-17 (31) — 인앱 자동 설치 실기 확인 (dev PC, v0.7.4 → v0.7.5)

- **v0.7.4 앱 MSI 설치**(msiexec /passive, UAC 승인) → 설치본 실행: 헤더 배지 '새 버전 v0.7.5' + 로그 "새 버전 v0.7.5 출시 (현재 v0.7.4)" — 실 릴리스 조회 정상.
- **발견: v0.7.4 에는 [지금 설치]가 없다.** 배지 클릭 시 1단계의 "다운로드 페이지를 열까요?" MessageBox 만 뜬다(2단계 코드는 v0.7.5 부터).
  → v0.7.4 → v0.7.5 는 다운로드 페이지에서 **한 번 수동 설치**가 맞음. #30 의 "v0.7.4 앱이 [지금 설치]로 자동" 서술과 v0.7.5 릴리스 노트를 정정(`gh release edit`).
  (자동화 스크립트가 없는 버튼 대신 '다운로드 페이지'를 눌러 브라우저가 열린 것을 사용자가 목격 — 앱 오동작 아님.)
- **v0.7.5 MSI 수동 설치**(MajorUpgrade 로 0.7.4 제거 확인, exit 0) → 설치본을 `BODA_CMS_VERSION=0.7.4` 로 가장 실행 → 배지 → 업데이트 창 → [지금 설치]:
  71.9MB 다운로드 진행률 → "무결성 확인 완료" → UAC 승인 → 앱 종료 → 부트스트랩: 종료 대기 5s → `msiexec` exit 0 → MSI 삭제 → `C:\Program Files\BODA.CMS\BODA.CMS.exe` 재실행(새 PID, 파일 버전 0.7.5)
  → 재실행 앱 헤더 v0.7.5·버튼 '업데이트 확인'(최신). 부트스트랩 로그·MSI 로그(ProgramData\BODA\CMS) 정상. **실제 UAC·msiexec 경로 검증 완료.**
  (같은 0.7.5 MSI 재설치라 버전 변화는 없음 — 버전 상승 증거는 다음 릴리스(v0.7.6)에서 자연히 확인.)
- dev PC 상태: BODA.CMS 모니터 v0.7.5 가 Program Files 에 설치된 채로 남김(시작 메뉴·바탕화면 바로가기 포함). 필요 시 앱 및 기능에서 제거.
- UIA 메모: 소유 창 안의 버튼은 `FindWindowW([NullString]::Value, 제목)` → `FromHandle` → 재시도 탐색 후 **마우스 좌표 클릭**이 가장 확실(InvokePattern·SetFocus+Space 는 포커스 상황에 따라 유실).
...
- **2단계가 들어간 첫 배포본.** 현장 흐름: v0.7.4 앱은 배지 → '다운로드 페이지' 로 이 MSI 를 받아 **한 번 수동 설치**(v0.7.4 에는 [지금 설치]가 없음 — 실기에서 확인, 아래 #31). v0.7.5 이상부터 배지 → [지금 설치] → UAC → 자동 설치·재실행. v0.7.3 이하는 알림 자체가 없어 수동 설치.
...
### 배경
- 사용자 보고: 3D 로봇이 실기(UR)와 다른 모양으로 움직임. `RobotStatusView.BuildRobot` 을 UR URDF(ur5e)와 대조한 결과 세 가지 불일치.
  1. **손목 축 배치**: 모델은 J4 = 전완 롤(Y), J5 = 피치(Z). 실제 UR 은 J2·J3·J4 가 평행 피치, J5 는 J4→J5 링크 방향 축, J6 은 J5 에 수직(=공구축).
     → 실기 손목1 굽힘이 화면에선 전완 비틀림으로, 손목2 회전이 손목 굽힘으로 표시됨.
  2. **영점 규약**: 모델은 θ=0 에서 링크 직진(수직 상향), UR 은 q=0 에서 팔 수평. 여기에 '제품 사진 자세'용 `RestOffsets {0,-35,60,0,30,0}` 를 더해 항상 어긋남
     (UR 홈 `[0,-90,0,-90,0,0]` 이 상완 125° 기운 자세로 그려짐).
  3. **클램프**: 시뮬레이터 사인파 억제용 `JointLimits {360,115,150,360,120,360}` 가 실기 각도에도 적용 — UR J2 의 -90~-180° 정상 구간이 -115° 에서 잘림.

### 작업 내용
- **손목 재구성** (`Views/RobotStatusView.xaml.cs`): J4 축 Z(피치), J4→J5 짧은 링크(+Y, d5), J5 축 Y(링크 방향), J5→J6 링크 측면(+Z, d6), J6 축 Z, 그리퍼 +Z 방향.
  치수는 UR5e ×0.62(상완 0.265·전완 0.245·d5/d6 0.062), 측면 오프셋도 실물처럼 지그재그(어깨 0 → 상완 바깥 → 전완 안쪽 → 손목1 바깥 → 플랜지).
- **벤더별 영점 오프셋** `ZeroOffsetsByVendor`: θ = q + offset. `ur`/`sim` = {0,90,0,90,0,0}, 미등록 벤더는 0(0° = 수직 상향 규약). 회전 방향은 우수 좌표계 매핑으로 UR 과 동일 — 부호 반전 없음.
- **클램프 제거** — 실기 각도는 그대로 표시. 시뮬레이터는 UR 규약 홈 `{0,-100,80,-70,90,0}` + 진폭을 실기 범위로 축소해 자기관통 없이 동작(`SimulatedRobotSource`).
- 베이스에 정적 요 45° — q1=0 에서 팔 평면이 기본 카메라 시선에 정면(형상 무관).

### 검증
- UR 홈 `[0,-90,0,-90,0,0]` ↔ 모델 θ 전부 0 → 팔 수직 상향 + 플랜지 측면 = 실기 홈 자세와 일치(전 관절 0 렌더로 확인).
- 시뮬레이터 UIA E2E: 제조사 전환 → 가상 Basic 시작 → 로봇 모드 스크린샷 — UR 작업 자세(팔꿈치 위·전완 접힘·손목 3축 지그재그)로 렌더, 자기관통 없음.
- 빌드 경고 0, 유닛테스트 77/77.

### 잔여
- 3D 는 UR 구조 기준. Doosan·Rokae·Jaka 는 오프셋 0 으로 자세 방향은 맞지만 손목 구조(롤-피치-롤)가 달라 손목 표현은 근사. 벤더별 손목 구조 분기는 필요 시.
- 실기 대조는 사용자 확인 필요(펜던트 자세 ↔ 화면). 다음 패키징(v0.7.3)에 포함.

---

## 2026-07-24 (26) — v0.7.2 배포 패키징 (온도 절대 임계 포함) + 릴리스 폴더 신설

- `tools\package.ps1 -Version 0.7.2` — 온도 절대 임계 판정(#25)이 들어간 첫 배포 패키지. 산출물 DLL 타임스탬프로 반영 확인.
- **현장 배포용 릴리스 폴더 신설: `D:\CMS-Releases`** — 배포 패키지 5종(setup 번들·MSI 2종·zip 2종)을 여기로 복사해 보관.
  dist 는 작업 폴더(최신 버전 + PostgreSQL 캐시만 유지), 구버전(v0.7.1)은 삭제.

---

## 2026-07-24 (25) — UR 실기 검증 + 온도 절대 임계 판정 전환

### 배경 (실기 발견)
- UR 실기 연결 성공 — 수 시간 연속 수신·CBM 동작 확인 (§5.5 URSim 게이트를 실기로 대체).
- **발견**: 연속 가동 시 관절 온도 웜업 때문에 건강도가 0까지 하락. 온도는 σ가 극히 작게 학습돼
  (하한 = 1% × |평균| ≈ 0.3~0.4℃) 정상 웜업 수십 ℃가 z 수십으로 폭주 — 미미한 블립에도 과민.
  z-점수 방식이 온도에 구조적으로 부적합하다는 실기 증거.

### 작업 내용
- **온도 신호만 절대 임계 판정으로 전환** (`CbmMonitor.EvaluateTemperature`):
  경고(기본 60℃) 아래는 건강도 영향 0 → 경고~알람(기본 75℃) 선형 감점 → 알람 지속 시 "과열" Alarm(디바운스),
  경고 이상 지속 시 "온도주의" Warning, 경고 아래 복귀 시 해제 통지. 순간 블립은 기존 디바운스로 무시.
- 건강도 환산은 의사 z(경고=1, 알람=5)로 기존 스냅숏·히트맵 경로 재사용 — UI 수정 없음.
- 설정: `Collector:Cbm:TemperatureWarnCelsius/TemperatureAlarmCelsius` (미설정 시 엔진 기본 60/75).
- `TelemetrySignals.TemperatureLabel` 상수화, 온도 알림은 ℃ 단위 전용 메시지.

### 검증
- 신규 테스트 5건: 웜업(30→55℃) 건강도 100 유지 · 경고~알람 선형 감점(67.5℃→50점) · 블립 디바운스 무시 ·
  과열 Alarm→복귀 · 설정 패스스루 — **77/77 통과, 빌드 경고 0**.

---

## 2026-07-23 (24) — v0.7.1 배포 패키징 (웹 대시보드 버튼 포함)

- `tools\package.ps1 -Version 0.7.1` — setup 번들(396.4MB)·앱/Collector MSI·zip 2종. 앱 버튼 동작은 실행 테스트로 확인(브라우저에서 대시보드 열림), 산출물 DLL 타임스탬프로 최신 소스 반영 확인.
- 구버전(v0.7.0) 패키지는 dist 에서 삭제 — dist 에는 최신 버전 + PostgreSQL 캐시만 유지하는 운영 방식.

---

## 2026-07-23 (23) — 앱 헤더 "웹 대시보드" 버튼

### 작업 내용
- 앱 헤더(AI 재학습 옆)에 **웹 대시보드** 버튼 추가 — 클릭 시 감시 서버 대시보드를 기본 브라우저로 연다.
- 주소 규칙은 `CollectorSync`와 단일화: 환경 변수 `BODA_COLLECTOR_URL` → 기본 `http://localhost:5100` (`CollectorSync.BaseUrl` 공개 프로퍼티 신설, MainWindow가 재사용).
- 열기 실패(연결된 브라우저 없음 등)는 경고 메시지박스로 안내.

### 검증
- 빌드 경고 0·오류 0, `dotnet test` 72/72 통과.
