...
- [x] **인앱 업데이트 2단계(자동 설치)**: `Services/UpdateInstallService` + `Views/UpdateWindow` — 앱 MSI 스트리밍 다운로드(진행률)·SHA-256 검증(GitHub asset digest)·UAC 부트스트랩 PowerShell(앱 종료 대기 → `msiexec /i /passive /norestart` + MSI 로그 → explorer 경유 재실행, 로그 `ProgramData\BODA\CMS\update-bootstrap.log`). **갱신 범위는 모니터 앱 MSI 뿐** — Collector(서비스)는 다른 PC 에 있을 수 있고 서비스 중단을 동반하므로 관리자가 collector MSI 로 별도 업그레이드(창에 명시). 검증용 `BODA_CMS_UPDATE_DRYRUN=1`(UAC·msiexec 생략). WORKLOG #30.
